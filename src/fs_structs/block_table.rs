...
pub enum BlockTableError {
    #[error("The block index is out of bounds. Block index provided: {0}, Max bit index: {1}")]
    OutOfBounds(u32, u32),
    #[error("File IO Error: {0}")]
    Io(#[from] std::io::Error),
    #[error("Type cast error: From {0} to {1}")]
    TypeCast(&'static str, &'static str),
...
impl BlockTable {
    ...
    fn new_table(size: u32) -> Result<Vec<BlockType>, BlockTableError> {
        Ok(vec![
            ...
            // errors if you cant
            size.try_into().map_err(|_| BlockTableError::TypeCast(
    ...
    pub fn create_and_write(dsfs: &Dsfs, group_index: GroupIndex) -> Result<Self, BlockTableError> {
        ...
        block_table.write(dsfs).map(|_| block_table)
    }
    ...
    pub fn from_fs(dsfs: &Dsfs, group_index: GroupIndex) -> Result<BlockTable, BlockTableError> {
        ...
        block_table.read(dsfs).map(|_| block_table)
    }

    /// Writes table state from memory to disk
    pub fn write(&self, dsfs: &Dsfs) -> Result<(), BlockTableError> {
        let block_index = match self.group_index {
            ...
            // groups block table.
            _ => dsfs.super_block.block_size * self.group_index,
        };
        Ok(dsfs.block_file.write_all_at(
            self.table.as_slice(),
            (block_index * dsfs.super_block.block_size).into(),
        )?)
    }

    /// Reads table state from disk to memory
    pub fn read(&mut self, dsfs: &Dsfs) -> Result<(), BlockTableError> {
        // if in the first group, block table is in block 1, otherwise, its in the first block of
        // the group (blocks_in_group * group_index)
        let block_index = match self.group_index {
            0 => 1,
            _ => dsfs.blocks_in_group * self.group_index,
        };
        Ok(dsfs.block_file.read_exact_at(
            &mut self.table,
            (block_index * dsfs.super_block.block_size).into(),
        )?)
    }
